...
let mem={employees:[],tasks:{},attend:{},allowedIPs:[],extLogs:[],notices:[],checked:{},reports:{},requests:[]};
const srv=http.createServer((req,res)=>{
  ...
  if(p==='/api/db'&&req.method==='GET'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(mem));return;}
  if(p==='/api/db'&&req.method==='POST'){
    let b='';req.on('data',c=>b+=c);
    req.on('end',()=>{try{mem=JSON.parse(b);res.writeHead(200);res.end('{"ok":true}');}catch{res.writeHead(400);res.end('{"error":"invalid"}');}});return;
  }
  if(p==='/api/attend'&&req.method==='POST'){
    ...
    req.on('end',()=>{
      try{
        ...
        if(!mem.attend)mem.attend={};if(!mem.attend[empId])mem.attend[empId]=[];
...
srv.listen(PORT,'0.0.0.0',()=>console.log('Team Board 시작! PORT:'+PORT));
